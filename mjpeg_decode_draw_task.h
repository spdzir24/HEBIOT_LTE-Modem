...
#define READ_BUFFER_SIZE 8192 //1024
// #define MAXOUTPUTSIZE (MAX_BUFFERED_PIXELS / 16 / 16)
#define MAXOUTPUTSIZE 9 //(288 / 3 / 16)
#define NUMBER_OF_DECODE_BUFFER 3 //3
#define NUMBER_OF_DRAW_BUFFER 9 //9
...
#define MAXOUTPUTSIZE    9   // 9 MCUs ~ halbe Zeile; bei RAM 18 testen
...
typedef struct {
  xQueueHandle xqh;         // decode input (mjpegBuf*)
  xQueueHandle drawQ;       // to send ready indices
  xQueueHandle freeQ;       // to acquire free indices
  JPEG_DRAW_CALLBACK *drawFunc;
} paramDecodeTask;

static JPEGDRAW jpegdraws[NUMBER_OF_DRAW_BUFFER];
static JPEGDEC _jpegDec;
static xQueueHandle _decode_in_q;
static xQueueHandle _draw_q;
static xQueueHandle _free_q;
static bool _useBigEndian;
...
// Draw-Callback: holt freien Index, kopiert Daten in exklusiven Slot, queued Index
static int queueDrawMCU(JPEGDRAW *pDraw)
{
  int idx;
  // Warte auf freien Slot
  xQueueReceive(_free_q, &idx, portMAX_DELAY);

  JPEGDRAW *j = &jpegdraws[idx];
  j->x = pDraw->x;
  j->y = pDraw->y;
  j->iWidth  = pDraw->iWidth;
  j->iHeight = pDraw->iHeight;

  const int len = pDraw->iWidth * pDraw->iHeight * 2;
  memcpy(j->pPixels, pDraw->pPixels, len);

  // Übergib Index an Draw-Task
  xQueueSend(_draw_q, &idx, portMAX_DELAY);
  return 1;
}
...
static void draw_task(void *arg)
{
  ...
  int idx;
  log_i("draw_task start.");
  while (xQueueReceive(p->drawQ, &idx, portMAX_DELAY))
  {
    JPEGDRAW *pd = &jpegdraws[idx];
    p->drawFunc(pd);

    // Slot nach dem Zeichnen wieder freigeben
    xQueueSend(p->freeQ, &idx, portMAX_DELAY);
  }
  vQueueDelete(p->drawQ);
  vQueueDelete(p->freeQ);
  log_i("draw_task end.");
  vTaskDelete(NULL);
}
                 ...
                 bool useBigEndian, BaseType_t decodeAssignCore, BaseType_t drawAssignCore)
{
  ...
  for (int i = 0; i < NUMBER_OF_DECODE_BUFFER; ++i) {
    _mjpegBufs[i].buf = (uint8_t *)malloc(mjpegBufSize);
    if (_mjpegBufs[i].buf) log_i("#%d decode buffer allocated.", i);
    else log_e("#%d decode buffer alloc failed.", i);
  }
  _mjpeg_buf = _mjpegBufs[_mBufIdx].buf;

  // SD-Lesepuffer
  if (!_read_buf) _read_buf = (uint8_t *)malloc(READ_BUFFER_SIZE);
  if (_read_buf) log_i("Read buffer allocated.");

  // Queues korrekt dimensionieren (Index-Übertragung)
  _draw_q  = xQueueCreate(NUMBER_OF_DRAW_BUFFER, sizeof(int));
  _free_q  = xQueueCreate(NUMBER_OF_DRAW_BUFFER, sizeof(int));
  _decode_in_q = xQueueCreate(NUMBER_OF_DECODE_BUFFER, sizeof(mjpegBuf*));

  // Free-Pool initial befüllen
  for (int i = 0; i < NUMBER_OF_DRAW_BUFFER; ++i) {
    // DMA-fähige Pixelpuffer pro Slot anlegen
    if (!jpegdraws[i].pPixels) {
      jpegdraws[i].pPixels = (uint16_t *)heap_caps_aligned_alloc(
        16, MAXOUTPUTSIZE * 16 * 16 * 2, MALLOC_CAP_DMA);
    }
    if (jpegdraws[i].pPixels) {
      log_i("#%d draw buffer allocated.", i);
    } else {
    ...
    // Index in Free-Pool
    xQueueSend(_free_q, &i, portMAX_DELAY);
  }

  // Task-Parameter
  _pDrawTask.drawQ   = _draw_q;
  _pDrawTask.freeQ   = _free_q;
  _pDrawTask.drawFunc = pfnDraw;

  _pDecodeTask.xqh = _decode_in_q;
  _pDecodeTask.drawQ = _draw_q;
  _pDecodeTask.freeQ = _free_q;
  _pDecodeTask.drawFunc = queueDrawMCU;

  // Tasks starten
  xTaskCreatePinnedToCore(
      (TaskFunction_t)decode_task, "MJPEG decode Task",
      4096, (void *)&_pDecodeTask,
      (UBaseType_t)configMAX_PRIORITIES - 1, &_decodeTask, decodeAssignCore);

  xTaskCreatePinnedToCore(
      (TaskFunction_t)draw_task, "MJPEG Draw Task",
      4096, (void *)&_pDrawTask,
      (UBaseType_t)configMAX_PRIORITIES - 1, &_draw_task, drawAssignCore);

  return true;
}

// MJPEG-Frame read bleibt unverändert (nur _decode_in_q verwenden)
...
bool mjpeg_draw_frame()
{
  ...
  // In Decode-Queue senden
  xQueueSend(_decode_in_q, &mBuf, portMAX_DELAY);

  // nächster Frame-Puffer
  ++_mBufIdx;
  if (_mBufIdx >= NUMBER_OF_DECODE_BUFFER) _mBufIdx = 0;
  _mjpeg_buf = _mjpegBufs[_mBufIdx].buf;
